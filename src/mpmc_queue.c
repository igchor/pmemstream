...
static uint64_t mpmc_queue_update_consumed_offset(struct mpmc_queue *manager, uint64_t expected_offset,
						  uint64_t desired_offset, uint64_t *ready_offset)
{
	if (desired_offset == expected_offset) {
		*ready_offset = desired_offset;
		return 0;
	}

	assert(desired_offset > expected_offset);

	bool weak = false;
	bool success = __atomic_compare_exchange_n(&manager->consume_offset, &expected_offset, desired_offset, weak,
						   __ATOMIC_RELEASE, __ATOMIC_RELAXED);
	*ready_offset = expected_offset;
	if (success) {
		return desired_offset - expected_offset;
	} else {
		return 0;
	}
}
