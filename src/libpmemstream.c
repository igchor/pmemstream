...
pmemstream_init(struct pmemstream *stream)
{
	...
	size_t metadata_size = MEMBER_SIZE(pmemstream_span_runtime, free);
...
{
	return (uint64_t *)((uint8_t *)stream->data->spans + offset);
}

static uint64_t*
pmemstream_get_span_for_offset_in_region(struct pmemstream_span_runtime region_sr, size_t offset)
{
	assert(region_sr.type == PMEMSTREAM_SPAN_REGION);
	return (uint64_t *)((uint8_t *)region_sr.data + offset);
}

static uint64_t
pmemstream_get_offset_for_span(struct pmemstream *stream, uint64_t *span)
{
...
{
	uint64_t *span = pmemstream_get_span_for_offset(stream, 0);
...
int
pmemstream_region_free(struct pmemstream *stream, struct pmemstream_region region)
{
	uint64_t *span = pmemstream_get_span_for_offset(stream, region.offset);
	struct pmemstream_span_runtime sr = pmemstream_span_get_runtime(span);
	...
	size_t metadata_size = MEMBER_SIZE(pmemstream_span_runtime, free);
	pmemstream_span_create_free(&span[0], stream->usable_size - metadata_size);
	stream->persist(&span[0], metadata_size);
			      ...
			      struct pmemstream_region region)
{
	...
	uint64_t *region_span = pmemstream_get_span_for_offset(stream, region.offset);
	struct pmemstream_span_runtime region_sr = pmemstream_span_get_runtime(region_span);
	assert(region_sr.type == PMEMSTREAM_SPAN_REGION);

	while (c->offset < region_sr.region.size) {
		uint64_t *span = pmemstream_get_span_for_offset_in_region(region_sr, c->offset);
		struct pmemstream_span_runtime sr = pmemstream_span_get_runtime(span);
		if (sr.type == PMEMSTREAM_SPAN_FREE) {
			break;
		}
	...
	struct pmemstream_entry *entry)
{
	...
	uint64_t *region_span = pmemstream_get_span_for_offset(stream, rcontext->region.offset);
	struct pmemstream_span_runtime region_sr = pmemstream_span_get_runtime(region_span);
	...
	uint64_t *entry_span = pmemstream_get_span_for_offset_in_region(region_sr, offset);
	if (entry) {
...
{
	uint64_t *entry_span = pmemstream_get_span_for_offset(stream, entry.offset);
...
int
pmemstream_region_iterator_next(struct pmemstream_region_iterator *it, struct pmemstream_region *region)
{
	uint64_t *region_span;
	struct pmemstream_span_runtime region_sr;
			       ...
			       struct pmemstream_entry *entry)
{
	for (;;) {
		...
		uint64_t *entry_span = pmemstream_get_span_for_offset_in_region(iter->region_sr, iter->offset);
