...
int pmemstream_from_map(struct pmemstream **stream, size_t block_size, struct pmem2_map *map)
{
	...
	// XXX
	sem_init(&s->async_ops_semaphore, 0, PMEMSTREAM_MAX_CONCURRENCY);
					    ...
					    size_t size)
{
	...
	stream->data.memset(destination + entry_total_size_span_aligned, 0, sizeof(struct span_entry),
			    PMEM2_F_MEM_NOFLUSH);
	stream->data.persist(destination, async_op->size + sizeof(struct span_entry));

	pmemstream_publish_timestamp(stream, region_runtime, timestamp);
