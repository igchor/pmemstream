...
auto make_offset_manager(size_t num_producers)
{
	return std::unique_ptr<offset_manager, decltype(&offset_manager_destroy)>(offset_manager_new(num_producers),
										  &offset_manager_destroy);
}
...
void mpmc_test_generic(const std::vector<uint32_t> &sizes, const unsigned num_producers, const unsigned num_consumers)
{
	...
	auto manager = make_offset_manager(num_producers);
	...
	parallel_exec(num_consumers + num_producers, [&](size_t tid) {
		if (tid < num_producers) {
			...
			while (!all_consumed.load(std::memory_order_relaxed)) {
				auto consumed_size = offset_manager_consume(manager.get(), &consumed_offset);
				if (consumed_size) {
	...
	return run_test([&] {
		...
		ret += rc::check(
			"verify if producer can consume it's own products", [](const std::vector<uint32_t> &sizes) {
				auto manager = make_offset_manager(1);

				uint64_t offset = 0;
				uint64_t validation_offset = 0;
				for (auto size : sizes) {
					offset = offset_manager_acquire(manager.get(), 0, size);
					RC_ASSERT(offset == validation_offset);
					validation_offset += size;

					/* Consume before calling produce should fail. */
					uint64_t ready_offset;
					size_t consumed_size = offset_manager_consume(manager.get(), &ready_offset);
					RC_ASSERT(consumed_size == 0);

					offset_manager_produce(manager.get(), 0);

					/* Now, it should succeed. */
					consumed_size = offset_manager_consume(manager.get(), &ready_offset);
