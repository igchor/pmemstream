...
auto make_pmemstream(const std::string &file, size_t block_size, size_t buffer_size, size_t size, bool truncate = true)
{
	...
	int ret = pmemstream_from_map(&stream, block_size, buffer_size, map);
	if (ret == -1) {
