	...
	void perform() override
	{
		...
		for (size_t i = 0; i < data.size() * sizeof(uint64_t); i += cfg.element_size) {
			if (region_t.offset != 0) {
				if (pmemstream_async_append(stream.get(), data_mover_sync_get_vdm(s), region_t, NULL,
							    data_chunks + i, cfg.element_size, NULL) < 0) {
					throw std::runtime_error("Error while appending " + std::to_string(i) +
								 " entry!");
				}
			} else {
				if (pmemstream_append(stream.get(), region_t, NULL, data_chunks + i, cfg.element_size,
						      NULL) < 0) {
					throw std::runtime_error("Error while appending " + std::to_string(i) +
								 " entry!");
				}
